...
def get_stat(month, date, wordle_number):
...
# Try with mobile user-agent
    ...
    try:
        retval = "Error, see console printout."
        ...
        if "Please verify you're human" in response.text:
            ...
            if "Today’s average difficulty" in response.text:
                ...
                end = response.text.find(".", start + 56)
                print(response.text[start:end])
                retval = response.text[start:end]
            else:
...
def main(month, date, wordle_number):
    get_stat(month, date, wordle_number)
